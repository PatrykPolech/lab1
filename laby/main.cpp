...
inline void DrawStar(Vector2 center, float outerRadius, float innerRadius, int numPoints, Color color) {
	const float angleStep = PI / numPoints; // 1/2 kąt między ramionami

	Vector2* points = new Vector2[numPoints * 2];
	for (int i = 0; i < numPoints * 2; ++i) {
		float radius = (i % 2 == 0) ? outerRadius : innerRadius;
		float angle = i * angleStep - PI / 2; // startuje od góry
		points[i] = { center.x + cosf(angle) * radius, center.y + sinf(angle) * radius };
	}

	for (int i = 0; i < numPoints * 2; ++i) {
		DrawLineV(points[i], points[(i + 1) % (numPoints * 2)], color);
	}
...
public:
	StarAsteroid(int w, int h) : Asteroid(w, h) { baseDamage = 20; }

	void Draw() const override {
		DrawStar(transform.position, GetRadius(), GetRadius() * 0.5f, 5, GRAY);
	}
};

class CircleAsteroid : public Asteroid {
public:
	CircleAsteroid(int w, int h) : Asteroid(w, h) { baseDamage = 8; }

	void Draw() const override {
		DrawCircleV(transform.position, GetRadius(), GRAY);
	}
};
...
static inline std::unique_ptr<Asteroid> MakeAsteroid(int w, int h, AsteroidShape shape) {
	...
	case AsteroidShape::CIRCLE:
		return std::make_unique<CircleAsteroid>(w, h);
	case AsteroidShape::STAR:
		return std::make_unique<StarAsteroid>(w, h);
	case AsteroidShape::RANDOM:
	default:
	{
		// Losujemy z prawdziwych kształtów, czyli 0 do 4
		int randomShape = GetRandomValue(0, 4);
		return MakeAsteroid(w, h, static_cast<AsteroidShape>(randomShape));
	}
	}
}
...
inline Asteroid* CreateRandomAsteroid(int screenW, int screenH) {
	int shape = GetRandomValue(0, 4); // 0–4 = 5 typów

	switch (shape) {
	case 0: return new TriangleAsteroid(screenW, screenH);
	case 1: return new SquareAsteroid(screenW, screenH);
	case 2: return new PentagonAsteroid(screenW, screenH);
	case 3: return new CircleAsteroid(screenW, screenH);
	case 4: return new StarAsteroid(screenW, screenH);
	default: return new TriangleAsteroid(screenW, screenH);
	}
}
...
public:
	...
	void Draw() const {
		if (type == WeaponType::BULLET) {
			DrawStar(transform.position, 8.f, 4.f, 5, YELLOW); // ⬅️ tu jest gwiazda
		}
		else {
	...
	float GetRadius() const {
		return (type == WeaponType::BULLET) ? 8.f : 4.f;
	}
			...
			// Shooting
			{
				if (player->IsAlive() && IsKeyDown(KEY_SPACE)) {
					...
					while (shotTimer >= interval) {
						...
						float spacing = 20.0f; // Odległość między działkami

						// Lewy pocisk
						Vector2 leftGun = { p.x - spacing, p.y };
						projectiles.push_back(MakeProjectile(currentWeapon, leftGun, projSpeed));

						// Prawy pocisk
						Vector2 rightGun = { p.x + spacing, p.y };
						projectiles.push_back(MakeProjectile(currentWeapon, rightGun, projSpeed));
